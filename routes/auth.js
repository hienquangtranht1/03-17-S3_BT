...
router.post('/changepassword', CheckLogin, async function (req, res, next) {
    try {
        ...
        if (!oldpassword || !newpassword) {
            return res.status(400).send("vui long nhap du thong tin (oldpassword, newpassword)");
        }
        if (newpassword.length < 6) {
            return res.status(400).send("mat khau moi phai tu 6 ky tu tro len");
        }
        let isValidOld = await userController.CompareLogin(user, oldpassword);
        if (!isValidOld) {
